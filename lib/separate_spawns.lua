...
function RemoveOrResetPlayer(player, remove_player, remove_force, remove_base,
    ...
    local player_old_force = player.force
    player.force = global.ocfg.main_force
    if global.markets[player.name] then global.markets[player.name] = market.new(player) end
    if global.groups[player.name] then global.groups[player.name] = group.new(player) end
    -- Clear globals
