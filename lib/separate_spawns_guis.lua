...
function WelcomeTextGuiClosedEvent(event)
    ...
    if (event.element and (event.element.name == "welcome_msg")) then
        ...
        player.opened = nil
        DisplaySpawnOptions(player)
    elseif (event.element and (event.element.name == "spawn_opts")) then
        local data = {
            element	= player.gui.screen.spawn_opts.spawn_solo_flow.spawn_solo_flow.isolated_spawn_near,
            player_index = event.player_index
        }
        SpawnOptsGuiClick(data)
...
function GetRandomSpawnPoint()
    local numSpawnPoints = table_size(global.ocore.sharedSpawns)
