...
Player_Init = {
    -- Time in Minutes
    AGE = {NOMAD = 20, IRON = 60, STEEL = 120},
...
function Player_Init.on_player_created(event)
    ...
    player.print({'msg-player-intro'})

    if player.admin then
        local color = Colors.admin
        player.color = color
        player.chat_color = color
    end
    game.print({'msg-new-player', player.name})
end
...
-- Alert player joined
-- @param event on_player_joined_game event
function Player_Init.on_player_joined_game(event)
    local player = game.players[event.player_index]
    local time_played = Time.tick_to_time_hms(player.online_time)
    if (player.online_time > 60) then
        time_played.string = string.format("%dh:%02dm", time_played.h,
                                           time_played.m)
        game.print({"msg-player-return", player.name, time_played.string})
    end
end
...
Event.register(defines.events.on_player_respawned,
               Player_Init.on_player_respawned)
...
Event.register(defines.events.on_player_joined_game,
               Player_Init.on_player_joined_game)
...
function Player_Init.addStartingItems(player)
    ...
    player.insert {name = 'iron-plate', count = 360}
    player.insert {name = 'copper-plate', count = 250}
    player.insert {name = 'pistol', count = 1}
...
function Player_Init.reveal_area(player)
    player.force.chart(player.surface, {
        {
            player.position.x - Player_Init.REVEAL_AREA_RADIUS,
            player.position.y - Player_Init.REVEAL_AREA_RADIUS
        }, {
            player.position.x + Player_Init.REVEAL_AREA_RADIUS,
            player.position.y + Player_Init.REVEAL_AREA_RADIUS
        }
