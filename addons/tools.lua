...
function tools.error(error_message, play_sound)
    ...
    game.player.print({'error.msg', error_message})
...
function tools.link_belts(player, inp, outp)
    if inp.valid and outp.valid then
        ...
        elseif inp.linked_belt_neighbour ~= outp then
            tools.error("Couldn't make link")
            return
        end
    else
        tools.error("Invalid")
...
function tools.make(player, sharedobject, flow)
    ...
    if sharedobject == "link" then
        ...
        if link_in and link_out then
            ...
            tools.error("Missing a link")
            return false
        end
    elseif sharedobject == "mode" then
        local sel = player.selected
        if not sel then
            tools.error("Place your cursor over the target linked belt.")
            return false
        end
        if sel.name == "linked-belt" then
            ...
            tools.error("Not a linked belt type.")
            return false
        end
    elseif sharedobject == "water" then
        local pos = GetWoodenChestFromCursor(player)
            ...
            tools.error("Failed to place waterfill. Don't stand so close FOOL!")
            return false
        end
    ...
    elseif shared_objects[sharedobject] then
        if flows[flow] then
            ...
            tools.error("Looking for 'in/out'")
            return
        end
    else
        tools.error("Invalid magic entity.. try /help make")
        return
    end
end
