...
local function on_gui_click(event)
    ...
    if el_name == Game_Info.MASTER_BUTTON_NAME or el_name ==
        'btn_gameinfo_close' then
...
function draw_gameinfo_frame(player)
    ...
    if (master_frame == nil) then
        ...
        local scrollable_content_frame = master_frame.add({
            type = 'scroll-pane',
        ...
        -- Flow
        local mapInfoSection = scrollable_content_frame.add({
            type = 'flow',
        ...
        -- Flow
        local button_flow = master_frame.add({
            type = 'flow',
            direction = 'horizontal'
        })
...
function draw_section(container, section_content_data)
    ...
    if (#section_content_data.title > 0) then
        local header = header_flow.add({
            type = 'label',
            caption = '=== ' .. section_content_data.title .. ' ==='
        })
